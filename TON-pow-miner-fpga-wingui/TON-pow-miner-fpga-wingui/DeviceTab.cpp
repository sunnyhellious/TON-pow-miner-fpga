...
void DeviceTab::MinerStart(void) {
	...
	std::string cmd_str = " -e \"pminer start " +
		...
		msclr::interop::marshal_as<std::string>(this->DevComTextBox->Text) + "\" ";
	...
	std::string fpga_miner_str = "\"" + AppSysSubProcess::ExePath() + "\\tools\\minertools\\tonlib-cli.exe" + "\"" + vl_str + glc_str + cmd_str;
	...
	if (CreateDirectoryA(wd.c_str(), NULL) || ERROR_ALREADY_EXISTS == GetLastError())
	{
		...
		if (this->DevTypeComboBox->SelectedIndex != 0) {
			...
			if( CopyFileA((AppSysSubProcess::ExePath() + "\\tools\\minertools\\sha256.cl").c_str(), (wd + "\\sha256.cl").c_str(), FALSE) == 0 )
				this->LogsTextBox->AppendText(gcnew String("Failed to copy OPENCL kernel\r\n"));

			if( CopyFileA((AppSysSubProcess::ExePath() + "\\tools\\minertools\\ocl_mul_factor_param.txt").c_str(), (wd + "\\ocl_mul_factor_param.txt").c_str(), FALSE) == 0 )
				this->LogsTextBox->AppendText(gcnew String("Failed to copy OPENCL MUL param txt\r\n"));
	...
	*/
	...
	AppSysSubProcess_0->Run(fpga_miner_str, "2>&1", wd, false);
