...
int MinerConfig::import_json_string(std::string json_string) {
	...
	*/
	...
	if (!mjson_get_number(json_chars, strlen(json_chars), "$.com_num", &double_tmp))
		return 1;
	this->com_num = double_tmp;
...
std::string MinerConfig::export_json_string(void) {
	...
	char *json_chars = mjson_aprintf("{ %Q:%d, %Q:%d, %Q:%d, %Q:%s, %Q:%d, %Q:%d, %Q:%d, %Q:%s }",
		// "type", this->type,
