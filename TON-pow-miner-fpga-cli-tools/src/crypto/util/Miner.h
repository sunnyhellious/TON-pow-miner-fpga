...
class Miner {
 public:
  struct Options {
    ...
    td::optional<td::Timestamp> expire_at = td::Timestamp::in(890);
    ...
    int verbosity;
    td::uint32 expire_base = (uint32_t)td::Clocks::system() + 900;
    td::int32 gpu_id;
...
struct HDataEnv {
  ...
  void init(const block::StdAddress& my_address, td::Slice seed) {
	...
	void init2(const block::StdAddress& my_address, td::Slice seed) {
    std::memcpy(body.myaddr, my_address.addr.data(), sizeof(body.myaddr));
    body.flags = static_cast<td::int8>(my_address.workchain * 4 + my_address.bounceable);
    CHECK(seed.size() == 16);
    std::memcpy(body.pseed, seed.data(), 16);
